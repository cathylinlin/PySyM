...
def test_states():
    ...
    # Test trajectory
    trajectory = [
        np.array([np.cos(t / 2), 1j * np.sin(t / 2)]) for t in np.linspace(0, np.pi, 20)
    ]
...
def test_wavefunction3d():
    ...
    from PySymmetry.visual import Wavefunction3DVisualizer, plot_orbital, plot_3d_slices
...
def test_entanglement():
    ...
    # Entropy evolution
    rho_t = [
        np.array([[(1 + np.sin(t)) / 2, 0], [0, (1 - np.sin(t)) / 2]], dtype=complex)
...
def test_quantum_ops():
    ...
    rho_t = [
        np.array(
            [
                [np.cos(t / 2) ** 2, np.cos(t / 2) * np.sin(t / 2)],
                [np.cos(t / 2) * np.sin(t / 2), np.sin(t / 2) ** 2],
            ],
            dtype=complex,
        )
