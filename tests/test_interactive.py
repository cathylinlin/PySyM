...
class TestParticle:
    def test_particle_creation(self):
        p = Particle(
            name="electron", mass=1.0, charge=-1.0, spin=0.5, position=[0.0, 0.0, 0.0]
        )
...
class TestPotential:
    ...
    def test_square_well(self):
        pot = Potential.square_well(
            center=np.array([0.0, 0.0, 0.0]), radius=1.0, depth=2.0
        )
    ...
    def test_gaussian_well(self):
        pot = Potential.gaussian_well(
            center=np.array([0.0, 0.0, 0.0]), depth=1.0, width=1.0
        )
...
class TestSceneHamiltonianBuilder:
    ...
    def test_build_kinetic_term_3d(self):
        scene = QuantumScene(
            name="Test", dimension=3, grid_points=3, grid_points_3d=[3, 3, 3]
        )
        builder = SceneHamiltonianBuilder(scene)
        T = builder.build_kinetic_term()
        assert T.shape[0] == 27
    ...
    def test_build_potential_term_3d(self):
        ...
        scene = QuantumScene(
            name="Test", dimension=3, grid_points_3d=[5, 5, 5], potentials=[pot]
        )
        builder = SceneHamiltonianBuilder(scene)
        V = builder.build_potential_term()
...
class TestSceneSymmetryAnalyzer:
    ...
    def test_translation_symmetry_periodic(self):
        scene = QuantumScene(
            name="Test", dimension=1, grid_points=20, boundary_condition="periodic"
        )
        analyzer = SceneSymmetryAnalyzer(scene)
        result = analyzer.analyze()
...
class TestSimulationResult:
    def test_result_summary(self):
        pot = Potential.harmonic(center=np.array([0.0]), k=1.0)
        scene = QuantumScene(
            name="Test", dimension=1, grid_points=100, potentials=[pot]
        )
...
class TestQuickSimulate:
    ...
    def test_quick_simulate_proton(self):
        result = quick_simulate(
            particles=[{"type": "proton", "position": [0.0]}], n_points=30
        )
        assert isinstance(result, SimulationResult)

    def test_quick_simulate_neutron(self):
        result = quick_simulate(particles=[{"type": "neutron"}], n_points=30)
        assert isinstance(result, SimulationResult)

    def test_quick_simulate_custom_particle(self):
        result = quick_simulate(
            particles=[
                {"type": "alpha", "mass": 4.0, "charge": 2.0, "position": [0.0]}
            ],
            n_points=30,
        )
        assert isinstance(result, SimulationResult)
    ...
    def test_quick_simulate_coulomb(self):
        result = quick_simulate(
            ...
            potentials=[
                {"type": "coulomb", "center": [0.0, 0.0, 0.0], "strength": 1.0}
            ],
            n_points=30,
        )
        assert isinstance(result, SimulationResult)
    ...
    def test_quick_simulate_square_well(self):
        result = quick_simulate(
            potentials=[
                {"type": "square_well", "center": [0.0], "radius": 1.0, "depth": 2.0}
            ],
            n_points=50,
        )
        assert isinstance(result, SimulationResult)
    ...
    def test_quick_simulate_custom_potential(self):
        result = quick_simulate(
            potentials=[
                {"type": "custom", "func": lambda x: x[0] ** 2, "name": "Quadratic"}
            ],
            n_points=50,
        )
        assert isinstance(result, SimulationResult)
