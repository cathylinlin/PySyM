...
class GroupHomomorphism:
    ...
    def __init__(self, domain: FiniteGroup, codomain: FiniteGroup, homomorphism: Callable[[GroupElement], GroupElement]):
        """
        初始化群同态
    ...
    def is_injective(self) -> bool:
        ...
        """
        # 检查核是否为平凡群
        kernel = [a for a in self.domain.elements() if self(a) == self.codomain.identity()]
        return len(kernel) == 1
...
class GroupAction(Generic[T]):
    ...
    def __init__(self, group: FiniteGroup, target_set: List[T], action: Callable[[GroupElement, T], T]):
        """
        初始化群作用
