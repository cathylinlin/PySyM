    ...
    def __init__(self, diagonal_or_matrix):
        ...
        if isinstance(diagonal_or_matrix, np.ndarray):
            if diagonal_or_matrix.ndim == 2:
                ...
                if np.allclose(diagonal_or_matrix, np.diag(np.diag(diagonal_or_matrix))):
                ...
                self._diagonal = np.array(diagonal_or_matrix, dtype=np.complex128 if np.iscomplexobj(diagonal_or_matrix) else np.float64)
                super().__init__(np.diag(diagonal_or_matrix))
            else:
                raise ValueError("输入必须是一维或二维数组")
        else:
            try:
                diagonal = np.array(diagonal_or_matrix, dtype=np.complex128 if any(isinstance(x, complex) for x in diagonal_or_matrix) else np.float64)
                if diagonal.ndim != 1:
