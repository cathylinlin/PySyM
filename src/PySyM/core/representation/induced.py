...
class InducedRepresentation(GroupRepresentation[T]):
    ...
    def __init__(self, group: Group[T], subgroup: Group[T], subgroup_representation: GroupRepresentation[T]):
        """
        Args:
            group: 群 G
            subgroup: 子群 H
        ...
        # 计算左陪集代表元
        self._coset_representatives = self._compute_coset_representatives(group, subgroup)
        self._coset_map = self._create_coset_map(group, subgroup, self._coset_representatives)
    ...
    def _compute_coset_representatives(self, group: Group[T], subgroup: Group[T]) -> List[T]:
    ...
    def _create_coset_map(self, group: Group[T], subgroup: Group[T], coset_representatives: List[T]) -> Dict[T, tuple[int, T]]:
    ...
    def induced_character(self, element: T) -> complex:
        ...
        for g_i in self._coset_representatives:
            ...
            conjugate = self._group.multiply(g_i_inv, self._group.multiply(element, g_i))
            
            if conjugate in self._subgroup.elements():
                # 获取子表示的特征标值
                character_sum += np.trace(self._subgroup_representation(conjugate).matrix)
