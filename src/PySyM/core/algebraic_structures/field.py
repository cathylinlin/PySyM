...
class ComplexField(Field[ComplexFieldElement]):
    ...
    def add(self, a: ComplexFieldElement, b: ComplexFieldElement) -> ComplexFieldElement:
        return a + b
    
    def multiply(self, a: ComplexFieldElement, b: ComplexFieldElement) -> ComplexFieldElement:
        return a * b
    
    def divide(self, a: ComplexFieldElement, b: ComplexFieldElement) -> ComplexFieldElement:
        return a / b
...
class ExtendedFiniteFieldElement(FieldElement):
    ...
    def _multiply_poly(self, a: List[int], b: List[int]) -> List[int]:
        ...
        for i, coeff_a in enumerate(a):
            for j, coeff_b in enumerate(b):
                result[i + j] = (result[i + j] + coeff_a * coeff_b) % self.characteristic
        # 对不可约多项式取模
    ...
    def _mod_poly(self, dividend: List[int], divisor: List[int]) -> List[int]:
        ...
        while degree_diff >= 0:
            # 计算商的首项系数
            leading_coeff = dividend[-1] * self._mod_inverse(divisor[-1], self.characteristic) % self.characteristic
            # 生成商多项式
    ...
    def __add__(self, other: 'ExtendedFiniteFieldElement') -> 'ExtendedFiniteFieldElement':
        if self.characteristic != other.characteristic:
            raise ValueError("有限域特征不同，无法运算")
        ...
        return ExtendedFiniteFieldElement(new_coeffs, self.characteristic, self.irreducible_poly)
    
    def __sub__(self, other: 'ExtendedFiniteFieldElement') -> 'ExtendedFiniteFieldElement':
        if self.characteristic != other.characteristic:
            raise ValueError("有限域特征不同，无法运算")
        if self.irreducible_poly != other.irreducible_poly:
            raise ValueError("不可约多项式不同，无法运算")
        ...
        neg_other = ExtendedFiniteFieldElement(neg_other_coeffs, self.characteristic, self.irreducible_poly)
        return self + neg_other
    
    def __mul__(self, other: 'ExtendedFiniteFieldElement') -> 'ExtendedFiniteFieldElement':
        if self.characteristic != other.characteristic:
            raise ValueError("有限域特征不同，无法运算")
        if self.irreducible_poly != other.irreducible_poly:
            raise ValueError("不可约多项式不同，无法运算")
    ...
    def __truediv__(self, other: 'ExtendedFiniteFieldElement') -> 'ExtendedFiniteFieldElement':
        if self.characteristic != other.characteristic:
            raise ValueError("有限域特征不同，无法运算")
        if self.irreducible_poly != other.irreducible_poly:
            raise ValueError("不可约多项式不同，无法运算")
    ...
    def __pow__(self, n: int) -> 'ExtendedFiniteFieldElement':
        if n < 0:
            n = -n
            return self.multiplicative_inverse() ** n
        result = ExtendedFiniteFieldElement([1], self.characteristic, self.irreducible_poly)  # 乘法单位元
        base = self
        while n > 0:
            if n % 2 == 1:
                result = result * base
    ...
    def inverse(self) -> 'ExtendedFiniteFieldElement':
        ...
        return ExtendedFiniteFieldElement(neg_coeffs, self.characteristic, self.irreducible_poly)
    ...
    def multiplicative_inverse(self) -> 'ExtendedFiniteFieldElement':
        ...
        while len(b) > 1 or (len(b) == 1 and b[0] != 0):
            ...
            # 计算商的首项系数
            leading_coeff = a[-1] * self._mod_inverse(b[-1], self.characteristic) % self.characteristic
            # 生成商多项式
            quotient = [0] * (degree_diff + 1)
            quotient[-1] = leading_coeff
            # 计算商与除数的乘积
            ...
            for i in range(len(quotient)):
                if i < len(y):
                    new_x[i + (len(new_x) - len(quotient))] = (new_x[i + (len(new_x) - len(quotient))] - quotient[i] * y[i]) % self.characteristic
            # 移除最高次项的零系数
        ...
        return ExtendedFiniteFieldElement(inv_coeffs, self.characteristic, self.irreducible_poly)
    
    def __hash__(self) -> int:
        return hash((tuple(self.coefficients), self.characteristic, tuple(self.irreducible_poly)))
    ...
    def additive_order(self) -> int:
        ...
        for d in range(1, self.characteristic + 1):
            # 计算 d * self
            result = ExtendedFiniteFieldElement([0], self.characteristic, self.irreducible_poly)
            temp = self
            for _ in range(d):
                result = result + temp
            if result.is_zero():
...
class ExtendedFiniteField(Field[ExtendedFiniteFieldElement]):
    ...
    def add(self, a: ExtendedFiniteFieldElement, b: ExtendedFiniteFieldElement) -> ExtendedFiniteFieldElement:
        return a + b
    
    def multiply(self, a: ExtendedFiniteFieldElement, b: ExtendedFiniteFieldElement) -> ExtendedFiniteFieldElement:
        return a * b
    
    def divide(self, a: ExtendedFiniteFieldElement, b: ExtendedFiniteFieldElement) -> ExtendedFiniteFieldElement:
        return a / b
    
    def inverse(self, a: ExtendedFiniteFieldElement) -> ExtendedFiniteFieldElement:
        return a.inverse()
    
    def multiplicative_inverse(self, a: ExtendedFiniteFieldElement) -> ExtendedFiniteFieldElement:
        return a.multiplicative_inverse()
    
    def identity(self) -> ExtendedFiniteFieldElement:
        return ExtendedFiniteFieldElement([0], self.characteristic, self.irreducible_poly)  # 加法单位元
    
    def zero(self) -> ExtendedFiniteFieldElement:
        return ExtendedFiniteFieldElement([0], self.characteristic, self.irreducible_poly)
    
    def one(self) -> ExtendedFiniteFieldElement:
        return ExtendedFiniteFieldElement([1], self.characteristic, self.irreducible_poly)  # 乘法单位元
    
    def __contains__(self, element: ExtendedFiniteFieldElement) -> bool:
        return (isinstance(element, ExtendedFiniteFieldElement) and 
                element.characteristic == self.characteristic and 
                element.irreducible_poly == self.irreducible_poly)
    ...
    def elements(self) -> List[ExtendedFiniteFieldElement]:
        ...
        def generate_polynomials(degree, current):
            if current is None:
                current = []
            if degree == 0:
                elements.append(ExtendedFiniteFieldElement(current.copy(), self.characteristic, self.irreducible_poly))
                return
