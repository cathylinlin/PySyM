...
from .phys_representation import PhysicalRepresentation
...
class SelectionRule(ABC):
    ...
    def is_allowed(self, 
                   initial_state: Any,
                   final_state: Any,
                   operator: Any = None) -> bool:
        """判断跃迁是否允许"""
        pass
    
    @abstractmethod
    def get_allowed_transitions(self,
                               states: List[Any]) -> List[Tuple[int, int]]:
        """获取所有允许跃迁"""
        pass
...
class ElectricDipoleSelectionRule(SelectionRule):
    ...
    def get_allowed_transitions(self,
                               states: List[Dict]) -> List[Tuple[int, int]]:
        """获取所有允许跃迁"""
        transitions = []
...
class GroupTheorySelectionRule(SelectionRule):
    ...
    def _direct_product(self, irrep1: str, irrep2: str) -> Dict[str, int]:
        ...
        for i, irrep in enumerate(self.irrep_names):
            # 内积计算重数
            multiplicity = self._inner_product(product_chars, 
    ...
    def _direct_product_contain(self, product: Dict[str, int], 
                                irrep: str) -> Dict[str, int]:
        """直积与第三个表示的乘积"""
        result = {}
    ...
    def get_allowed_transitions(self,
                               ...
                               operator_irrep: str) -> List[Tuple[int, int]]:
        """
        获取所有允许跃迁
    ...
    def transition_intensity(self,
                           initial_irrep: str,
                           final_irrep: str,
                           operator_components: List[str]) -> float:
        """
        估计跃迁强度
...
class VibrationSelectionRule(SelectionRule):
    ...
    def analyze_vibrational_modes(self, 
                                 mode_irreps: List[str]) -> Dict[str, List[int]]:
        """
        分析振动模式的光谱活性
...
class ComprehensiveSelectionRules:
    ...
    def __init__(self, point_group: Optional[PointGroup] = None):
    ...
    def analyze_transition(self,
                          initial_state: Dict,
                          final_state: Dict,
                          transition_type: str = 'electric_dipole') -> Dict[str, Any]:
        """
        综合分析跃迁
