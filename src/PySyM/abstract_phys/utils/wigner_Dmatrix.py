...
from math import factorial, sqrt, cos, sin, exp, pi
...
class WignerSmallD:
    ...
    def _wigner_formula(j: float, m_prime: float, m: float, beta: float) -> float:
        ...
        prefactor = sqrt(
            ...
            factorial(int(j + m_prime)) * factorial(int(j - m_prime))
        )
        
        sum_term = 0.0
        for k in range(k_min, k_max + 1):
            numerator = ((-1)**(k + m - m_prime) * 
    ...
    @staticmethod
    def symmetry_relations(j: float, m_prime: float, m: float, beta: float) -> Dict[str, float]:
        """
        对称性关系
...
class WignerBigD:
    ...
    @staticmethod
    def compute(j: float, m_prime: float, m: float,
        ...
        # 相位因子
        phase = exp(-1j * m_prime * alpha) * exp(-1j * m * gamma)
...
class SphericalHarmonicsRotation:
    ...
    @staticmethod
    def rotate_harmonic(l: int, m: int, 
                        alpha: float, beta: float, gamma: float) -> Dict[int, complex]:
        """
        旋转球谐函数
    ...
    @staticmethod
    def rotation_matrix_for_l(l: int, 
