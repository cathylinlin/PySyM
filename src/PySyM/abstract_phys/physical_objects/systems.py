...
from PySyM.core.group_theory.product_group import DirectProductGroup
...
class ClassicalSystem(PhysicalSystem):
                ...
                for other_particle in self._particles:
                    if particle != other_particle:
                        for interaction in self._interactions:
                            total_force += interaction.calculate_force(particle, other_particle)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'name': self._name,
            'particles': [{'mass': p.get_mass(), 'charge': p.get_charge(), 'spin': p.get_spin(),
                         'position': p.position.tolist(), 'velocity': p.velocity.tolist()}
    ...
    def get_total_energy(self):
        ...
        for i, particle in enumerate(self._particles):
            ...
            for j in range(i + 1, len(self._particles)):
                for interaction in self._interactions:
                    potential_energy += interaction.calculate_potential(particle, self._particles[j])
...
class RelativisticSystem(PhysicalSystem):
    ...
    def equations_of_motion(self, *args, **kwargs) -> Any:
        ...
        accelerations = []
        for particle in self._particles:
            if hasattr(particle, 'four_position') and hasattr(particle, 'four_velocity'):
                acceleration = self._spacetime.geodesic_equation(
                    particle.four_position, 
...
class CompositeSystem(PhysicalSystem):
    ...
    def is_invariant_under(self, operation: SymmetryOperation) -> bool:
        ...
        return all(subsystem.is_invariant_under(operation) for subsystem in self._subsystems)
    
    def equations_of_motion(self, *args, **kwargs) -> Any:
        """运动方程"""
        # 返回所有子系统的运动方程
        return [subsystem.equations_of_motion(*args, **kwargs) for subsystem in self._subsystems]
...
class HamiltonianSystem(PhysicalSystem):
    ...
    def equations_of_motion(self, q: np.ndarray, p: np.ndarray, t: float = 0) -> Tuple[np.ndarray, np.ndarray]:
        """
        哈密顿方程
        ...
        for i in range(len(p)):
            ...
            p_minus = p.copy()
            p_minus[i] -= epsilon
            dq_dt[i] = (self._hamiltonian(q, p_plus, t) - 
                       self._hamiltonian(q, p_minus, t)) / (2 * epsilon)
        ...
        for i in range(len(q)):
            ...
            q_minus = q.copy()
            q_minus[i] -= epsilon
            dp_dt[i] = -(self._hamiltonian(q_plus, p, t) - 
                        self._hamiltonian(q_minus, p, t)) / (2 * epsilon)
    ...
    def is_invariant_under(self, operation: SymmetryOperation) -> bool:
        ...
        # 简化版：数值验证
        test_points = [
            (np.random.randn(self._dim), np.random.randn(self._dim))
            for _ in range(10)
        ]
    ...
    def poisson_bracket(self, f: Callable, g: Callable, 
...
class LagrangianSystem(PhysicalSystem):
    ...
    def equations_of_motion(self, q: np.ndarray, dq: np.ndarray, t: float = 0) -> np.ndarray:
        """
        欧拉-拉格朗日方程
        ...
        for i in range(self._dim):
            ...
            q_minus = q.copy()
            q_minus[i] -= epsilon
            dL_dq[i] = (self._lagrangian(q_plus, dq, t) - 
                       self._lagrangian(q_minus, dq, t)) / (2 * epsilon)
            ...
            dq_minus = dq.copy()
            dq_minus[i] -= epsilon
            dL_ddq[i] = (self._lagrangian(q, dq_plus, t) - 
                        self._lagrangian(q, dq_minus, t)) / (2 * epsilon)
        ...
        for i in range(self._dim):
            for j in range(self._dim):
                ...
                dq_minus = dq.copy()
                dq_minus[j] -= epsilon
                dL_ddq_plus = (self._lagrangian(q, dq_plus, t) - 
                              self._lagrangian(q, dq_minus, t)) / (2 * epsilon)
                
                dq_plus[i] += epsilon
                dq_minus[i] += epsilon
                dL_ddq_plus_i = (self._lagrangian(q, dq_plus, t) - 
                               self._lagrangian(q, dq_minus, t)) / (2 * epsilon)
                
                dq_plus[i] -= 2 * epsilon
                dq_minus[i] -= 2 * epsilon
                dL_ddq_minus_i = (self._lagrangian(q, dq_plus, t) - 
                                self._lagrangian(q, dq_minus, t)) / (2 * epsilon)
    ...
    def energy(self, q: np.ndarray, dq: np.ndarray, t: float = 0) -> float:
        ...
        for i in range(self._dim):
            ...
            dq_minus = dq.copy()
            dq_minus[i] -= epsilon
            p[i] = (self._lagrangian(q, dq_plus, t) - 
                   self._lagrangian(q, dq_minus, t)) / (2 * epsilon)
