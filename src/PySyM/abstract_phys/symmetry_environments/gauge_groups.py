...
from ...core.matrix_groups.base import MatrixGroup, MatrixGroupElement
from ...core.matrix_groups.special_linear import SpecialLinearGroup, SLnElement
...
class SU3GaugeGroup(GaugeGroup):
    ...
    LAMBDA_8 = np.array([[1, 0, 0], [0, 1, 0], [0, 0, -2]], dtype=np.complex128) / np.sqrt(3)
    GELL_MANN_MATRICES = [
    ...
    def structure_constants(self) -> np.ndarray:
        ...
        for i in range(8):
            for j in range(8):
                commutator = self.GELL_MANN_MATRICES[i] @ self.GELL_MANN_MATRICES[j] - \
                            self.GELL_MANN_MATRICES[j] @ self.GELL_MANN_MATRICES[i]
                for k in range(8):
                    f[i, j, k] = np.imag(np.trace(commutator @ self.GELL_MANN_MATRICES[k]))
        return f
