...
class Potential:
    ...
    def coulomb(cls, center: np.ndarray, strength: float, Z: float = 1.0):
        ...
        def V(x):
            ...
            if r < epsilon:
                r = epsilon
            return -Z * strength / r
        return cls(name='Coulomb', potential_type='coulomb', dimension=dim,
    ...
    def harmonic(cls, center: np.ndarray, k: float):
        ...
        def V(x):
            ...
            return 0.5 * k * r_sq
        return cls(name='Harmonic', potential_type='harmonic', dimension=dim,
    ...
    def square_well(cls, center: np.ndarray, radius: float, depth: float):
        ...
        return cls(name='SquareWell', potential_type='square_well', dimension=dim,
                   parameters={'center': center, 'radius': radius, 'depth': depth},
                   function=V)
    
    @classmethod
    def harmonic_3d(cls, center: np.ndarray = None, kx: float = 1.0, ky: float = None, kz: float = None):
        """3D各向异性谐振子势"""
        if center is None:
        ...
        return cls(name='Harmonic3D', potential_type='harmonic_3d', dimension=3,
    ...
    def spherical_well(cls, radius: float, depth: float, center: np.ndarray = None):
        ...
        def V(x):
            r = np.linalg.norm(x - center)
            return -depth if r < radius else 0.0
        return cls(name='SphericalWell', potential_type='spherical_well', dimension=3,
                   parameters={'center': center, 'radius': radius, 'depth': depth},
                   function=V)
    ...
    def gaussian_well(cls, center: np.ndarray, depth: float, width: float = 1.0):
        ...
        return cls(name='GaussianWell', potential_type='gaussian_well', dimension=dim,
                   parameters={'center': center, 'depth': depth, 'width': width},
                   function=V)
    
    @classmethod
    def cylindrical_potential(cls, axis: str = 'z', radius: float = 1.0, depth: float = 1.0):
        """柱对称势能"""
        def V(x):
            if axis == 'z':
        ...
        return cls(name='Cylindrical', potential_type='cylindrical', dimension=3,
    ...
    def step(cls, position: float, height: float):
        ...
        return cls(name='Step', potential_type='step', dimension=1,
    ...
    def custom(cls, func: Callable, name: str = 'Custom', dimension: int = 3):
        """自定义势能"""
        return cls(name=name, potential_type='custom', dimension=dimension, function=func)
...
class QuantumScene:
    ...
    spatial_range_3d: List[Tuple[float, float]] = field(default_factory=lambda: [(-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0)])
    dimension: int = 1
    grid_points: int = 100
    ...
    symmetry_info: Optional[Dict[str, Any]] = field(default_factory=dict, repr=False)
...
class SceneBuilder:
    ...
    def add_harmonic_3d(self,
                        center: List[float] = None,
                        kx: float = 1.0,
                        ky: float = None,
                        kz: float = None) -> 'SceneBuilder':
        """添加3D各向异性谐振子势"""
    ...
    def add_spherical_well(self,
                           radius: float,
                           depth: float,
                           center: List[float] = None) -> 'SceneBuilder':
        """添加球方势阱"""
        c = np.array(center) if center else np.array([0.0, 0.0, 0.0])
    ...
    def add_gaussian_well(self,
                          center: List[float],
                          depth: float,
                          width: float = 1.0) -> 'SceneBuilder':
        """添加高斯势阱"""
    ...
    def set_spatial_range_3d(self,
                              x_range: Tuple[float, float],
                              y_range: Tuple[float, float] = None,
                              z_range: Tuple[float, float] = None) -> 'SceneBuilder':
        """设置3D空间范围"""
        if y_range is None:
            y_range = x_range
        if z_range is None:
    ...
    def set_grid_points_3d(self,
                           nx: int,
                           ny: int = None,
                           nz: int = None) -> 'SceneBuilder':
        """设置3D网格点数"""
        if ny is None:
            ny = nx
        if nz is None:
...
class SceneSymmetryAnalyzer:
    ...
    def _check_translation_symmetry_3d(self) -> bool:
        ...
        if self._scene.boundary_condition == 'periodic':
            ...
            self._symmetry_matrices['translation_x'] = self._build_translation_matrix_3d(1, 0, 0)
            self._symmetry_matrices['translation_y'] = self._build_translation_matrix_3d(0, 1, 0)
            self._symmetry_matrices['translation_z'] = self._build_translation_matrix_3d(0, 0, 1)
            return True
        return False
    ...
    def _check_spherical_symmetry(self) -> bool:
        """检查球对称性"""
        for pot in self._scene.potentials:
            if pot.potential_type in ('coulomb', 'spherical_well', 'harmonic_3d', 'gaussian_well'):
    ...
    def _build_translation_matrix_3d(self, shift_x: int = 0, shift_y: int = 0, shift_z: int = 0) -> np.ndarray:
        """构建3D平移矩阵"""
        gx, gy, gz = self._grid_3d
        nx, ny, nz = len(gx), len(gy), len(gz)
        N = nx * ny * nz
    ...
    def classify_states_by_symmetry(self, states: List[Any], energies: np.ndarray) -> Dict[str, Any]:
        """根据对称性对态进行分类"""
        classifications = {
            'parity': {},
            'quantum_numbers': {}
...
def simulate(scene: QuantumScene, num_states: int = 5, analyze_symmetry: bool = True) -> 'SimulationResult':
    """
    模拟量子场景
    ...
    """
    from .solver import QuantumSolverFactory, ExactDiagonalizationSolver, HydrogenAtomSolver, HarmonicOscillatorSolver
    ...
    if isinstance(solver, (HydrogenAtomSolver, HarmonicOscillatorSolver)):
        ...
        if analyze_symmetry and sym_analyzer is not None:
            try:
                sym_classifications = sym_analyzer.classify_states_by_symmetry(states, energies)
            except Exception:
                sym_classifications = None
...
class SimulationResult:
    ...
    def get_parity(self, state_index: int) -> Optional[int]:
        """获取态的宇称"""
        if self.symmetry_analysis and 'parity' in self.symmetry_analysis:
