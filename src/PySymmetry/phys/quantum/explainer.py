...
class EnergySpectrumExplainer(ResultExplainer):
    ...
    def compute_spectrum_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "first_excited_energy": excited_energies[0]
    ...
    def _compute_degeneracies(
        self, energies: np.ndarray, tol: float = 1e-6
    ) -> dict[float, int]:
        """计算简并度"""
        degeneracies = {}
        for e in energies:
    ...
    def explain(self, spectrum_data: dict[str, Any] | None = None) -> str:
        """生成能谱解释"""
...
class QuantumStateExplainer(ResultExplainer):
    ...
    def __init__(self, state: Ket | DensityMatrix):
    ...
    def _explain_mixed_state(self) -> dict[str, Any]:
        """混合态分析"""
        assert isinstance(self._state, DensityMatrix), (
            "Mixed state must be a DensityMatrix"
        )
        rho = self._state
    ...
    def explain(self, state_data: dict[str, Any] | None = None) -> str:
        """生成态解释"""
...
class DynamicsExplainer(ResultExplainer):
    ...
    def explain(self, dynamics_data: dict[str, Any] | None = None) -> str:
        """生成动力学解释"""
...
class MeasurementExplainer(ResultExplainer):
    ...
    def __init__(
        self, measurement_results: list[int], basis_labels: list[str] | None = None
    ):
    ...
    def explain(self, stats: dict[str, Any] | None = None) -> str:
        """生成测量解释"""
        if stats is None:
...
class DecoherenceExplainer(ResultExplainer):
    ...
    def explain(self, decoherence_data: dict[str, Any] | None = None) -> str:
        """生成退相干解释"""
...
def explain_quantum_system(
    hamiltonian: HamiltonianOperator,
    state: Ket | DensityMatrix,
    state_history: list[Ket] | None = None,
...
class SymmetryExplainer(ResultExplainer):
    ...
    def __init__(
        ...
        state_classifications: dict[str, Any] | None = None,
    ):
    ...
    def explain(self, symmetry_data: dict[str, Any] | None = None) -> str:
        """生成对称性解释"""
...
def explain_measurement_results(
    measurement_results: list[int], basis_labels: list[str] | None = None
) -> str:
    """
    解释测量结果
