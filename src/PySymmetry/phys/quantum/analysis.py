...
class QuantumAnalyzer:
    ...
    def __init__(
        self,
        hamiltonian: HamiltonianOperator | None = None,
        result: SimulationResult | None = None,
    ):
        self._H = hamiltonian
        self._result = result

        self._parity_op: QuantumParityOperation | None = None
        self._translation_op: QuantumTranslationOperation | None = None

        self._abstract_analyzer: SymmetryAnalyzer | None = None
        ...
        for sym in symmetries:
            if sym.name == "Parity":
                ...
                conserved["momentum"] = {
                    ...
                    "generator": MomentumGenerator(1)
                    if _HAS_ABSTRACT_PHYS
                    else "Spatial translation",
                }
    ...
    def compute_invariants(self) -> dict[str, float]:
        ...
        if len(self._result.energies) >= 2:
            invariants["energy_gap"] = float(
                abs(self._result.energies[1] - self._result.energies[0])
            )
    ...
    def generate_report(self) -> str:
        ...
        if classifications:
            lines.append(
                f"{'n':<4} {'E':<12} {'Parity':<8} {'Irrep':<6} {'Quantum Numbers'}"
            )
            for cls in classifications[:8]:
                p = (
                    f"{cls.parity:+.0f}"
                    if isinstance(cls.parity, (int, float))
                    else "?"
                )
                lines.append(
                    f"{cls.index:<4} {cls.energy:<12.4f} {p:<8} {cls.irrep:<6} {cls.quantum_numbers}"
                )
        ...
        lines.append("-" * 40)
        lines.append(
            f"  Allowed: {len(allowed)}, Forbidden: {len(rules) - len(allowed)}"
        )
        if allowed[:3]:
            lines.append("  Sample allowed transitions:")
            for r in allowed[:3]:
                lines.append(
                    f"    |{r.initial}> -> |{r.final}>: dE = {r.energy_gap:.4f}"
                )
...
def analyze(
    hamiltonian: HamiltonianOperator, result: SimulationResult
) -> AnalysisResult:
    """
...
def quick_report(hamiltonian: HamiltonianOperator, result: SimulationResult) -> str:
    """
...
class GroupRepresentation:
    ...
    def __init__(self, name: str, dimension: int):
        ...
        self.character: np.ndarray | None = None
    ...
    def is_irreducible(self) -> bool:
        ...
        return (
            np.abs(
                np.vdot(self.character, self.character) - len(self.conjugacy_classes)
            )
            < 1e-10
        )
...
class LieAlgebra:
    ...
    def __init__(self, name: str, dimension: int):
        ...
        self.structure_constants: np.ndarray | None = None
    ...
    def is_representation(self, matrices: list[np.ndarray]) -> bool:
        ...
        for i in range(self.dimension):
            for j in range(self.dimension):
                ...
                rhs = sum(
                    self.structure_constants[i, j, k] @ matrices[k]
...
class GroupTheoryAnalyzer:
    ...
    def __init__(self, hamiltonian: HamiltonianOperator | None = None):
        self._H = hamiltonian
    ...
    def detect_symmetry_group(
        self, H_matrix: np.ndarray, tolerance: float = 1e-8
    ) -> list[str]:
        """检测哈密顿量的对称群"""
        groups = []
    ...
    def classify_irreps(
        self, energies: np.ndarray, states: list[Ket], tolerance: float = 1e-6
    ) -> dict[int, str]:
        """根据能级简并度分类不可约表示"""
        classifications = {}
...
def analyze_group_theory(
    hamiltonian: HamiltonianOperator, energies: np.ndarray, states: list[Ket]
) -> dict[str, Any]:
    """
    群论分析便捷函数
...
def wigner_eckart_coupling(
    j1: float, j2: float, j3: float, m1: float, m2: float, m3: float, q: int
) -> float:
    """
    Wigner-Eckart定理计算
...
def tensor_operator_matrix_element(
    j: float, k: int, q: int, j_prime: float, m: float, m_prime: float
) -> float:
    """
    张量算符矩阵元
...
class SO4HydrogenAnalyzer:
    ...
    def detect_so4_symmetry(self, tolerance: float = 1e-8) -> dict[str, Any]:
        ...
        if hasattr(self._symmetry_analyzer, "verify_h_commutes_with_generators"):
            result["commutation_verified"] = (
                self._symmetry_analyzer.verify_h_commutes_with_generators(tolerance)
            )
    ...
    def analyze_so4_structure(self) -> dict[str, Any]:
        ...
        if hasattr(self._symmetry_analyzer, "analyze_degeneracy_structure"):
            structure["degeneracy_structure"] = (
                self._symmetry_analyzer.analyze_degeneracy_structure()
            )
    ...
    def generate_report(self) -> str:
        ...
        if "error" not in structure:
            ...
            lines.append("-" * 50)
            lines.append(
                f"{'n':<4} {'E_n (a.u.)':<15} {'Degeneracy':<12} {'SO(4) Rep'}"
            )
            lines.append("-" * 50)
            ...
            if "degeneracy_structure" in structure:
                for n, data in structure["degeneracy_structure"].items():
                    lines.append(
                        f"{n:<4} {data['energy']:<15.6f} {data['degeneracy']:<12} {data['so4_representation']}"
                    )
            lines.append("-" * 50)
