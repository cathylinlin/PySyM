...
class QuantumToAbstractBridge:
    ...
    def to_physical_system(self) -> PhysicalSystem:
        ...
        return PhysicalSystem(
            name=self._abstract_system.name
            if self._abstract_system
            else "QuantumSystem",
            system_type="quantum",
            dimension=self._abstract_system.hilbert_space_dim
            if self._abstract_system
            else 0,
        )
...
class SymmetryToQuantumBridge:
    ...
    def get_operator(self, name: str) -> np.ndarray | None:
        """获取对称算符"""
...
class LieAlgebraToQuantumBridge:
    ...
    def heisenberg_xxx_hamiltonian(self, J: float, n_sites: int) -> np.ndarray:
        ...
        for i in range(n_sites - 1):
            term = np.kron(
                np.kron(np.eye(2**i), sx @ sx + sy @ sy + sz @ sz),
                np.eye(2 ** (n_sites - i - 2)),
            )
            H += J * term
...
class GroupRepresentationBridge:
    ...
    def create_irreducible_representation(
        self, group_name: str, basis_size: int
    ) -> np.ndarray:
        """
        创建不可约表示矩阵
