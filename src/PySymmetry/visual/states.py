...
class BlochSphere:
    ...
    def __init__(
        self, figsize: tuple[float, float] = (10, 10), title: str = "Bloch Sphere"
    ):
        _check_matplotlib()
    ...
    def add_state(
        ...
        if arrow:
            self._axes.quiver(
                0, 0, 0, x, y, z, color=color, arrow_length_ratio=0.1, linewidth=2
            )
...
def bloch_vector(ket: np.ndarray) -> np.ndarray:
    ...
    rho = np.array([[abs(a) ** 2, a * np.conj(b)], [b * np.conj(a), abs(b) ** 2]])
...
def plot_bloch_trajectory(
    trajectory: list[np.ndarray], label: str = "Trajectory", color: str = "blue"
) -> BlochSphere:
    """
    绘制Bloch球上的演化轨迹
    ...
    for i, state in enumerate(trajectory):
        bs.add_state(
            state, label=f"t={i}" if i % 10 == 0 else None, color=color, arrow=False
        )
