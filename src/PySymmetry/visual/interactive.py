...
class InteractivePlotter:
    ...
    def energy_levels(
        ...
        energies: np.ndarray,
        labels: list[str] | None = None,
        title: str = "Interactive Energy Levels",
    ) -> "go.Figure":
        """
        创建交互式能级图
    ...
    def probability_3d(
        ...
        values: np.ndarray,
        isolevel: float = 0.1,
        title: str = "3D Probability Density",
    ) -> "go.Figure":
        """
        创建交互式3D概率密度
        ...
        fig.update_layout(
            title=title, xaxis_title="X", yaxis_title="Y", height=600, width=800
        )

        return fig

    def wavefunction_animation(
        self,
        x: np.ndarray,
        states: list[np.ndarray],
        ...
        for i, (t, state) in enumerate(zip(times, states)):
            ...
            fig.add_trace(
                go.Scatter(
                    x=x, y=psi, mode="lines", name=f"t={t:.2f}", visible=(i == 0)
                )
            )
    ...
    def dashboard(
        ...
        densities: list[np.ndarray] | None = None,
        title: str = "Quantum System Dashboard",
    ) -> "go.Figure":
        """
        创建量子系统仪表盘
...
def plotly_energy_levels(
    energies: np.ndarray, labels: list[str] | None = None, title: str = "Energy Levels"
) -> "go.Figure":
    """
    Plotly 能级图
