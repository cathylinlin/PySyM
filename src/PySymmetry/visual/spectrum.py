...
class EnergySpectrumPlotter:
    ...
    def plot_levels(
        ...
        for i, E in enumerate(energies):
            ymin = (
                min(energies) - 0.1 * abs(min(energies))
                if min(energies) != max(energies)
                else -1
            )
            ymax = (
                max(energies) + 0.1 * abs(max(energies))
                if min(energies) != max(energies)
                else 1
            )
    ...
    def plot_ladder(
        self,
        energies: np.ndarray,
        energies_2: np.ndarray | None = None,
    ...
    def plot_spectrum_with_degeneracy(
        self,
        energies: np.ndarray,
        degeneracies: list[int] | None = None,
        title: str = "Energy Spectrum with Degeneracy",
    ) -> tuple:
        """
        绘制带简并度的能谱
    ...
    def plot_diagram(
        self,
        energies: np.ndarray,
        transitions: list[tuple[int, int, float]] | None = None,
        title: str = "Energy Level Diagram",
    ) -> tuple:
        """
        绘制能级跃迁图
