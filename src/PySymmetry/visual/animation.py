...
class StateEvolutionAnimator:
    ...
    def animate_bloch_sphere(self, states: List[np.ndarray],
    ...
    def animate_probability_density(self, x: np.ndarray,
    ...
    def animate_density_matrix(self, states: List[np.ndarray],
                              times: Optional[List[float]] = None,
        ...
        im = ax.imshow(np.real(states[0]), cmap='RdBu_r', vmin=-1, vmax=1,
                      animated=True)
    ...
    def animate_energy_levels(self, energies: List[np.ndarray],
                            times: Optional[List[float]] = None,
        ...
        bars = ax.bar(range(len(energies[0])), energies[0], color='steelblue', alpha=0.8)
        ...
        ani = FuncAnimation(fig, update, frames=len(energies),
                           init_func=init, interval=interval, blit=False)
...
def animate_bloch(states: List[np.ndarray],
                 times: Optional[List[float]] = None,
                 title: str = "Bloch Sphere Evolution",
                 save_path: Optional[str] = None) -> FuncAnimation:
    """
    ...
    return animator.animate_bloch_sphere(states, times, title=title, save_path=save_path)


def animate_probability(x: np.ndarray,
                       states: List[np.ndarray],
                       times: Optional[List[float]] = None,
    ...
    """
    animator = StateEvolutionAnimator()
    return animator.animate_probability_density(x, states, times, title, save_path=save_path)


def create_rabi_oscillation_animation(Omega: float = 1.0,
                                      T: float = 10.0,
                                      n_points: int = 100) -> FuncAnimation:
    """
    创建Rabi振荡动画
    ...
    animator = StateEvolutionAnimator()
    return animator.animate_bloch_sphere(states, times.tolist(), 
