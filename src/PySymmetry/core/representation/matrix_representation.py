...
同态约定：对所有 ``a,b ∈ G`` 有 ``ρ(ab) = ρ(a) @ ρ(b)``（与 :meth:`~PySymmetry.core.group_theory.Group.multiply` 中
...
class MatrixRepresentation(GroupRepresentation[T]):
    ...
    def change_basis(self, basis: np.ndarray) -> "MatrixRepresentation":
        ...
        if basis.shape != (self.dimension, self.dimension):
            raise ValueError(
                "Basis matrix must be square with dimension equal to representation dimension"
            )
