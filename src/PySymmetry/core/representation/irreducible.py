...
class IrreducibleRepresentationFinder:
    ...
    def find_all(group: Group) -> list[GroupRepresentation]:
        ...
        # 检查是否为 S3 群
        if len(elements) == 6 and IrreducibleRepresentationFinder._is_standard_s3(
            elements
        ):
            # 构造平凡表示
        ...
        # 对于其他群，使用原始方法
        # 1. 首先构造正则表示
        regular_rep = IrreducibleRepresentationFinder._construct_regular_representation(
            group
        )
        ...
        # 3. 去重，确保每个不可约表示只出现一次
        unique_irreducible_reps = (
            IrreducibleRepresentationFinder._unique_representations(irreducible_reps)
        )
    ...
    def decompose(representation: GroupRepresentation) -> list[GroupRepresentation]:
        ...
        # 2. 找到所有不可约特征标
        irreducible_chars = (
            IrreducibleRepresentationFinder._find_irreducible_characters(group)
        )
    ...
    @staticmethod
    def _find_irreducible_characters(
        group: Group,
    ) -> list[tuple[Character, GroupRepresentation]]:
        """找到群的所有不可约特征标"""
        # 实现有限群的不可约特征标查找
        ...
        while reps_to_decompose and len(irreducible_reps) < num_classes:
            ...
            if current_char.is_irreducible():
                ...
                for existing_char in irreducible_chars:
                    if all(
                        abs(current_char(g) - existing_char(g)) < 1e-10
                ...
                try:
                    rep1, rep2 = IrreducibleRepresentationFinder._split_representation(
                        current_rep
                    )
            ...
            for subgroup in subgroups:
                if len(subgroup.elements()) < len(group.elements()):
                    ...
                    for subgroup_irrep in subgroup_irreps:
                        ...
                        induced_rep = InducedRepresentation(
                            group, subgroup, subgroup_irrep
                        )
                        # 分解诱导表示
                        induced_decomposition = (
                            IrreducibleRepresentationFinder.decompose(induced_rep)
                        )
                        for rep in induced_decomposition:
                            ...
                            if rep_char.is_irreducible():
                                ...
                                for existing_char in irreducible_chars:
                                    if all(
                                        abs(rep_char(g) - existing_char(g)) < 1e-10
                                        for g in group.elements()
                                    ):
                                        is_new = False
                                        break
    ...
    ) -> tuple[GroupRepresentation, GroupRepresentation]:
        """将可约表示分解为两个表示的直和"""
        # 这里使用随机方法来找到不变子空间
        # 实际应用中应该使用更系统的方法
        group = representation.group
    ...
    def _find_subgroups(group: Group) -> list[Group]:
        ...
        if len(elements) == 6 and IrreducibleRepresentationFinder._is_standard_s3(
            ...
            # 找到 A3 子群（偶置换）
            a3_elements = [
                g for g in elements if g in [(0, 1, 2), (1, 2, 0), (2, 0, 1)]
            ]
    ...
    @staticmethod
    def _unique_representations(
        representations: list[GroupRepresentation],
        ...
        for rep in representations:
            ...
            for seen_char in seen_chars:
                if all(
                    abs(char(g) - seen_char(g)) < 1e-10 for g in rep.group.elements()
                ):
                    is_unique = False
                    break
