...
class FiniteDimensionalVectorSpaceElement(VectorSpaceElement):
    ...
    def __add__(
        self, other: "FiniteDimensionalVectorSpaceElement"
        ...
        new_components = [
            self.field.add(a, b) for a, b in zip(self.components, other.components)
        ]
    ...
    def __sub__(
        ...
        if self.field != other.field:
            raise ValueError("向量空间基础域不匹配")
        new_components = [
            self.field.subtract(a, b) for a, b in zip(self.components, other.components)
        ]
        return FiniteDimensionalVectorSpaceElement(new_components, self.field)
...
class FiniteDimensionalVectorSpace(VectorSpace[FiniteDimensionalVectorSpaceElement, F]):
    ...
    def __init__(self, field: Field[F], dimension: int, name: str = ""):
        ...
        """
        super().__init__(
            field, name or f"{field.name} Vector Space of dimension {dimension}"
        )
        self._dimension = dimension

    def add(
        self,
        a: FiniteDimensionalVectorSpaceElement,
        b: FiniteDimensionalVectorSpaceElement,
    ) -> FiniteDimensionalVectorSpaceElement:
        return a + b
    ...
    def __contains__(self, element: FiniteDimensionalVectorSpaceElement) -> bool:
        return (
            isinstance(element, FiniteDimensionalVectorSpaceElement)
            and len(element.components) == self._dimension
            and element.field == self.field
        )
...
class LinearTransformation:
    ...
    def __init__(
        self,
        domain: VectorSpace[M, F],
        codomain: VectorSpace[M, F],
        matrix: list[list[F]],
    ):
        """
        初始化线性变换
        ...
        if len(matrix) != codomain_dim:
            raise ValueError(
                f"矩阵行数必须等于陪域维度，期望{codomain_dim}，实际{len(matrix)}"
            )
        for row in matrix:
            if len(row) != domain_dim:
                raise ValueError(
                    f"矩阵列数必须等于定义域维度，期望{domain_dim}，实际{len(row)}"
                )
        ...
        for row in self.matrix:
            ...
            for i, scalar in enumerate(row):
                ...
                if hasattr(vector, "components"):
                    component = self.codomain.field.add(
                        component,
                        self.codomain.field.multiply(scalar, vector.components[i]),
                    )
                else:
    ...
    def __add__(self, other: "LinearTransformation") -> "LinearTransformation":
        ...
        for i in range(len(self.matrix)):
            ...
            for j in range(len(self.matrix[0])):
                row.append(
                    self.codomain.field.add(self.matrix[i][j], other.matrix[i][j])
                )
            matrix.append(row)
    ...
    def compose(self, other: "LinearTransformation") -> "LinearTransformation":
        ...
        for i in range(m):
            ...
            for j in range(n):
                ...
                for k in range(p):
                    elem = self.codomain.field.add(
                        elem,
                        self.codomain.field.multiply(
...
class TensorProduct(Module[TensorProductElement, R]):
    ...
    def add(
        self, a: TensorProductElement, b: TensorProductElement
    ) -> TensorProductElement:
        return a + b

    def scalar_multiply(
        self, a: TensorProductElement, scalar: R
    ) -> TensorProductElement:
        return a * scalar
    ...
    def tensor(self, elements: list[ModuleElement]) -> TensorProductElement:
        ...
        if len(elements) != len(self.modules):
            raise ValueError(
                f"元素数量必须等于模块数量，期望{len(self.modules)}，实际{len(elements)}"
            )

        # 检查每个元素是否属于对应的模块
        for elem, module in zip(elements, self.modules):
            if elem not in module:
                raise ValueError("元素不属于对应的模块")
