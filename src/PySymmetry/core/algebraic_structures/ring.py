...
class IntegerRing(Ring[IntegerRingElement]):
    ...
    def multiply(
        self, a: IntegerRingElement, b: IntegerRingElement
    ) -> IntegerRingElement:
        return a * b
...
class PolynomialRing(Ring[PolynomialRingElement]):
    ...
    def add(
        self, a: PolynomialRingElement, b: PolynomialRingElement
...
class MatrixRingElement(RingElement):
    ...
    def __post_init__(self):
        ...
        if len(self.entries) != self.rows:
            raise ValueError(
                f"矩阵行数不匹配，期望{self.rows}行，实际{len(self.entries)}行"
            )
        ...
        for i in range(self.rows):
            ...
            for j in range(other.cols):
                ...
                for k in range(self.cols):
                    entry = self.ring.add(
                        entry,
                        self.ring.multiply(self.entries[i][k], other.entries[k][j]),
                    )
    ...
    def __pow__(self, n: int) -> "MatrixRingElement":
        ...
        # 初始化为单位矩阵
        result = MatrixRingElement(
            [
                [
                    self.ring.one() if i == j else self.ring.zero()
...
class MatrixRing(Ring[MatrixRingElement]):
    ...
    def identity(self) -> MatrixRingElement:
        # 加法单位元（零矩阵）
        zero_entries = [
            [self.ring.zero() for _ in range(self.n)] for _ in range(self.n)
        ]
    ...
    def one(self) -> MatrixRingElement:
        # 乘法单位元（单位矩阵）
        identity_entries = [
            [self.ring.one() if i == j else self.ring.zero() for j in range(self.n)]
    ...
    def __contains__(self, element: MatrixRingElement) -> bool:
        return (
            isinstance(element, MatrixRingElement)
            and element.ring == self.ring
            and element.rows == self.n
            and element.cols == self.n
        )
