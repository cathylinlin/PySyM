...
class MatrixFactory:
    ...
    @staticmethod
    def from_function(
        rows: int, cols: int, func: Callable[[int, int], float]
    ) -> np.ndarray:
        """从函数构造矩阵
    ...
    def toeplitz(first_row: np.ndarray, first_col: np.ndarray = None) -> np.ndarray:
        ...
        return np.linalg.toeplitz(
            first_col if first_col is not None else first_row, first_row
        )
    ...
    @staticmethod
    def tridiagonal(
        diagonal: np.ndarray, upper: np.ndarray, lower: np.ndarray
    ) -> np.ndarray:
        """三对角矩阵
