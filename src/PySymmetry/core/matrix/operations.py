...
class MatrixOperations:
    ...
    @staticmethod
    def inner_product(a: np.ndarray, b: np.ndarray) -> float | complex:
        """内积
    ...
    @staticmethod
    def norm(matrix: np.ndarray, ord: int | float | str = "fro") -> float:
        """矩阵范数
    ...
    @staticmethod
    def split(
        matrix: np.ndarray, indices_or_sections: int, axis: int = 0
    ) -> list[np.ndarray]:
        """矩阵分割
