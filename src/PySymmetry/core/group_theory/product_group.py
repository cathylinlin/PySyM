...
class ProductGroup(FiniteGroup, ABC, Generic[T]):
    ...
    def _get_components_from_index(
        self, index: int
...
class DirectProductGroupElement(ProductGroupElement):
    ...
    def __mul__(
        self, other: "DirectProductGroupElement"
        ...
        return self.group._get_element_from_components(
            self.group.group1.multiply(g1, g2), self.group.group2.multiply(h1, h2)
        )
...
class DirectProductGroup(ProductGroup[DirectProductGroupElement]):
    ...
    def _element_mul(
        self, a: DirectProductGroupElement, b: DirectProductGroupElement
    ) -> DirectProductGroupElement:
        """直积群的乘法实现"""
        return a * b
    ...
    def _get_element_from_components(
        self, g: GroupElement, h: GroupElement
    ) -> DirectProductGroupElement:
        """根据两个群的元素创建直积群的元素"""
    ...
    def conjugacy_classes(self) -> list[list[DirectProductGroupElement]]:
        ...
        for c1 in classes1:
            for c2 in classes2:
                # 构建笛卡尔积
                class_elements = [
                    self._get_element_from_components(n, h) for n in c1 for h in c2
                ]
...
class SemidirectProductGroupElement(ProductGroupElement):
    ...
    def __mul__(
        self, other: "SemidirectProductGroupElement"
...
class SemidirectProductGroup(ProductGroup[SemidirectProductGroupElement]):
    ...
    def _element_mul(
        self, a: SemidirectProductGroupElement, b: SemidirectProductGroupElement
    ) -> SemidirectProductGroupElement:
        """半直积群的乘法实现"""
        return a * b
    ...
    def _get_element_from_components(
        self, n: GroupElement, h: GroupElement
    ) -> SemidirectProductGroupElement:
        """根据两个群的元素创建半直积群的元素"""
    ...
    def _get_inverse(
        self, element: SemidirectProductGroupElement
    ) -> SemidirectProductGroupElement:
        """获取半直积群元素的逆元"""
                ...
                for g in self.group1.elements():
                    for k in self.group2.elements():
                        ...
                        n_conj = self.group1.multiply(
                            g, self.phi(k)(self.group1.multiply(n, self.phi(h)(g_inv)))
                        )
                        h_conj = self.group2.multiply(self.group2.multiply(k, h), k_inv)
                        class_elements.append(
                            self._get_element_from_components(n_conj, h_conj)
                        )
                classes.append(class_elements)
        return classes
