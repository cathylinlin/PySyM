...
T = TypeVar("T", bound="GroupElement")  # 群元素类型
...
class Group(ABC, Generic[T]):
    ...
    def __init__(self, name: str = ""):
        ...
        self._properties: GroupProperties | None = None
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                for a in elements:
                    for b in elements:
                        # 交换子 [a, b] = a*b*a^{-1}*b^{-1}
                        commutator = current_group.multiply(
                            a,
                            current_group.multiply(
                                b,
                                current_group.multiply(
                                    current_group.inverse(a), current_group.inverse(b)
                                ),
                            ),
                        )
                ...
                # 生成导群
                derived_subgroup = current_group.generate_subgroup(
                    list(derived_elements)
                )
