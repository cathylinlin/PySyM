...
from .abstract_group import FiniteGroup, Group, GroupElement
...
class Coset(GroupElement, Generic[T]):
    ...
    def elements(self) -> list[T]:
        ...
        if self._elements is None:
            if self.is_left:
                self._elements = [
                    self.group.multiply(self.representative, h)
                    for h in self.subgroup.elements()
                ]
            else:
                self._elements = [
                    self.group.multiply(h, self.representative)
                    for h in self.subgroup.elements()
                ]
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if (
            ...
            or self.subgroup != other.subgroup
            or self.is_left != other.is_left
        ):
            return False
