...
class PoincareTransformation(SymmetryOperation):
    ...
    def __init__(self, lorentz: LorentzTransformation, translation: np.ndarray = None):
    ...
    def compose(self, other: "PoincareTransformation") -> "PoincareTransformation":
        ...
        new_translation = self.lorentz.matrix @ other.translation + self.translation
    ...
    def act_on_field(self, field_value: np.ndarray, position: FourVector) -> np.ndarray:
        """
        作用于场
    ...
    def __repr__(self) -> str:
        return (
            f"PoincareTransformation(lorentz={self.lorentz.transformation_type()}, "
...
class PoincareGroup(PhysicalSymmetry):
    ...
    def create_operation(self, params: SymmetryParameters) -> PoincareTransformation:
        ...
        lorentz_params = SymmetryParameters(
            continuous_params=cont_params.get("lorentz", {})
        )
    ...
    def lie_algebra(self) -> dict[str, Any]:
        ...
        """
        return {
            "translations": {"commutation": "[Pμ, Pν] = 0", "abelian": True},
            "rotations": {"commutation": "[Ji, Jj] = iεijk Jk", "so3_subalgebra": True},
            "rotation_translation": {"commutation": "[Ji, Pj] = iεijk Pk"},
            "boost_translation": {
                "commutation": "[Ki, P₀] = -iPi, [Ki, Pj] = -iδij P₀"
            },
            "boosts": {"commutation": "[Ki, Kj] = -iεijk Jk"},
        }
...
class PauliLubanskiVector:
    ...
    @staticmethod
    def compute(momentum: FourVector, angular_momentum: np.ndarray) -> FourVector:
        """
        计算Pauli-Lubanski矢量
            ...
            for nu in range(4):
                for alpha in range(4):
                    for beta in range(4):
                        W[mu] += (
                            (1 / 2)
                            * epsilon[mu, nu, alpha, beta]
                            * angular_momentum[nu, alpha]
                            * momentum.components[beta]
                        )
...
class FieldTransformation:
    ...
    @staticmethod
    def scalar_field_transformation(
        field_value: float, transformation: PoincareTransformation, position: FourVector
    ) -> float:
        """
        标量场变换
    ...
    @staticmethod
    def vector_field_transformation(
    ...
    @staticmethod
    def spinor_field_transformation(
        field_value: np.ndarray,
        transformation: PoincareTransformation,
        position: FourVector,
    ) -> np.ndarray:
...
class RelativisticScattering:
    ...
    @staticmethod
    def mandelstam_variables(
        p1: FourVector, p2: FourVector, p3: FourVector, p4: FourVector
    ) -> dict[str, float]:
        """
        Mandelstam变量
    ...
    @staticmethod
    def two_body_decay(m_parent: float, m1: float, m2: float) -> dict[str, float]:
        """
        两体衰变运动学
    ...
    @staticmethod
    def cross_section_kinematics(
        s: float, m1: float, m2: float, m3: float, m4: float
    ) -> dict[str, float]:
        """
        散射截面的运动学因子
        ...
        # 初态动量
        lambda_i = (
            s**2 + m1**4 + m2**4 - 2 * s * m1**2 - 2 * s * m2**2 - 2 * m1**2 * m2**2
        )
        ...
        # 末态动量
        lambda_f = (
            s**2 + m3**4 + m4**4 - 2 * s * m3**2 - 2 * s * m4**2 - 2 * m3**2 * m4**2
        )
