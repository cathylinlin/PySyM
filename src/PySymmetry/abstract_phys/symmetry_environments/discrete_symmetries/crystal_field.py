...
from .point_groups import PointGroup, OhGroup, TdGroup
...
class CrystalFieldParameters:
    ...
    def to_dict(self) -> Dict[str, float]:
        return {
            'B': self.B, 'C': self.C,
            'B20': self.B20, 'B40': self.B40, 'B44': self.B44,
            'B60': self.B60, 'B64': self.B64,
            'Dq': self.Dq,
            'B22': self.B22, 'B42': self.B42, 'B43': self.B43,
            'zeta': self.zeta
        }
...
class CrystalFieldPotential:
    ...
    def _C(self, k: int, q: int, theta: float, phi: float) -> float:
        ...
        if q == 0:
            ...
            # 实组合
            C_real = (sph_harm(q, k, phi, theta) + 
                     sph_harm(-q, k, phi, theta)) / np.sqrt(2)
...
class CrystalFieldLevelCalculator:
    ...
    def _assign_terms(self, eigenvalues: np.ndarray, 
                     eigenvectors: np.ndarray) -> List[str]:
        """指派光谱项"""
        terms = []
...
class TanabeSuganoDiagram:
    ...
    def generate(self, 
                ...
                n_points: int = 100,
                B: float = 1000.0,
                C_B_ratio: float = 4.0) -> Dict[str, np.ndarray]:
        """
        生成Tanabe-Sugano图数据
    ...
    def find_spin_crossover(self) -> Optional[float]:
        """寻找自旋交叉点（如果存在）"""
...
class LigandFieldTheory:
    ...
    def _project_to_irrep(self, irrep: str) -> Optional[np.ndarray]:
        """投影到不可约表示"""
        # 使用投影算符
        # P^Γ = (d_Γ/|G|) Σ_g χ^Γ(g)* R(g)
        
        char_table, irrep_names, _ = self.point_group.character_table_full()
        irrep_idx = irrep_names.index(irrep)
    ...
    def dd_transitions(self, params: CrystalFieldParameters) -> List[Dict]:
        ...
        for i, (E, term) in enumerate(zip(energies, terms)):
            ...
            transition = {
                ...
                'wavelength': 1e7 / transition_energy if transition_energy > 0 else np.inf,  # cm^-1 to nm
