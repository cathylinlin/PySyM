...
class Molecule:
    ...
    def center_of_mass(self, masses: list[float] | None = None) -> np.ndarray:
        """质心"""
        if masses is None:
    ...
    def inertia_tensor(self, masses: list[float] | None = None) -> np.ndarray:
        """转动惯量张量"""
        if masses is None:
            masses = [self._get_atomic_mass(atom.symbol) for atom in self.atoms]
...
class MolecularSymmetryDetector:
    ...
    def _get_possible_axis_directions(
    ...
    def _is_rotation_axis(
        self, positions: list[np.ndarray], axis: np.ndarray, order: int
    ) -> bool:
        """检查是否是n重旋转轴"""
    ...
    def _has_horizontal_mirror(
        self, positions: list[np.ndarray], rotation_axes: list[dict]
    ) -> bool:
        """检查是否有水平镜面"""
        if not rotation_axes:
            return False
...
class MolecularOrbitalSymmetry:
    ...
    def symmetry_adapted_linear_combination(
        self, atomic_orbitals: list[tuple[int, str]]
    ) -> dict[str, np.ndarray]:
        """
        构造对称性匹配线性组合（SALC）
    ...
    def orbital_symmetry_labels(self, orbital_coefficients: np.ndarray) -> list[str]:
        """
        确定分子轨道的对称性标签
