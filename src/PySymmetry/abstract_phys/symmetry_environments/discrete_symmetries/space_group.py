...
class BravaisLattice:
    ...
    def reciprocal_lattice(self) -> "BravaisLattice":
        ...
        return BravaisLattice(lattice_type=self.lattice_type, lattice_vectors=b)
...
class SpaceGroupOperation:
    ...
    def inverse(self) -> "SpaceGroupOperation":
        ...
        return SpaceGroupOperation(
            rotation=R_inv, translation=-R_inv @ self.translation
        )
...
class SpaceGroup:
    ...
    def _parse_operations(self, ops_data: list[dict]) -> list[SpaceGroupOperation]:
        """解析操作列表"""
        return [
            SpaceGroupOperation(rotation=op["rotation"], translation=op["translation"])
            for op in ops_data
        ]
    ...
    def generate_symmetry_equivalent_positions(
        self, position: np.ndarray, lattice: BravaisLattice
    ) -> list[np.ndarray]:
        """
        生成对称等价位置
...
class BrillouinZone:
    ...
    def k_path(
        self, path: list[str] = None, n_points: int = 50
