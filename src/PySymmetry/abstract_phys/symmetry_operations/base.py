...
from typing import Protocol, Any, Optional, TYPE_CHECKING, Union, TypeVar
import numpy as np

if TYPE_CHECKING:
    from PySymmetry.core.group_theory.abstract_group import Group
    from PySymmetry.abstract_phys.physical_objects.abstract_physical_objects import PhysicalObject
...
class SymmetryOperation(ABC):
    ...
    def __init__(self):
        ...
        self._representation_dim: Optional[int] = None
    ...
    def representation_matrix(self, dim: Optional[int] = None) -> np.ndarray:
        """获取该对称操作的表示矩阵
    ...
    def is_unitary(self, tol: float = 1e-10) -> bool:
        ...
        return bool(np.allclose(matrix @ matrix.conj().T, np.eye(matrix.shape[0]), atol=tol))
