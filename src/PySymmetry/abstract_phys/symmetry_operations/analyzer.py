...
class SymmetryAnalyzer:
    ...
    def detect_symmetries(self, 
                         tolerance: float = 1e-8) -> List['PhysicalSymmetry']:
        """
        自动检测系统对称性
    ...
    def _check_time_translation(self, tolerance: float) -> bool:
        ...
        if hasattr(self.system, 'is_time_dependent') and self.system.is_time_dependent():
            return False
        return True
    ...
    def decompose_state(self, 
                       state: 'State',
                       symmetry: 'PhysicalSymmetry') -> Dict[str, float]:
        """
        将态分解为对称性本征态
    ...
    def selection_rules(self,
                       ...
                       operator_irrep: 'IrreducibleRepresentation') -> bool:
        """
        判断跃迁是否被选择定则允许
    ...
    def _tensor_product(self, irrep1: Any, 
                       irrep2: Any) -> List['IrreducibleRepresentation']:
        """计算表示的张量积"""
    ...
    def analyze_spectrum(self,
        ...
        for i, (E, indices) in enumerate(degenerate_groups):
            ...
            irrep = self._identify_irreducible_representation(
                eigenvectors[:, indices] if len(indices) > 1 else eigenvectors[:, indices].reshape(-1, 1)
            )
            analysis[f'level_{i}'] = {
                'energy': E,
    ...
    def _find_degenerate_levels(self, eigenvalues: np.ndarray, 
                               tol: float = 1e-8) -> List[tuple]:
        """找出简并能级"""
        degenerate_groups = []
    ...
    def _identify_irreducible_representation(self, 
                                           eigenvectors: np.ndarray) -> Optional[str]:
        """识别不可约表示"""
        n = eigenvectors.shape[1] if len(eigenvectors.shape) > 1 else 1
        return f"dim_{n}"
    
    def _extract_quantum_numbers(self, irrep: Optional[str]) -> Dict[str, Any]:
        """提取量子数"""
