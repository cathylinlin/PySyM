...
class GaugeOperation(SymmetryOperation):
    ...
    def compose(self, other):
        if isinstance(other, GaugeOperation) and self._group == other._group:
            # 通过群乘法组合规范操作的参数
            if (
                hasattr(self._group, "multiply")
                and hasattr(self._params, "continuous_params")
                and hasattr(other._params, "continuous_params")
            ):
                try:
                    combined_params = self._group.multiply(
                        self._params.continuous_params.get(
                            "element", self._group.identity()
                        ),
                        other._params.continuous_params.get(
                            "element", self._group.identity()
                        ),
                    )
    ...
    def inverse(self):
        if hasattr(self._group, "inverse") and hasattr(
            self._params, "continuous_params"
        ):
            try:
                elem = self._params.continuous_params.get(
                    "element", self._group.identity()
                )
                inv_elem = self._group.inverse(elem)
                new_params = SymmetryParameters(continuous_params={"element": inv_elem})
                return GaugeOperation(self._group, new_params)
            except (AttributeError, TypeError):
                pass
        return GaugeOperation(self._group, self._params)
...
class CompositeOperation(SymmetryOperation):
    ...
    def compose(self, other):
        if isinstance(other, CompositeOperation) and len(self._operations) == len(
            other._operations
        ):
            new_ops = [
                op1.compose(op2)
...
class TranslationSymmetry(PhysicalSymmetry):
    ...
    def create_operation(self, params: SymmetryParameters) -> "TranslationOperation":
        """创建平移操作"""
        displacement = params.continuous_params.get(
            "displacement", np.zeros(self._dimension)
        )
...
class RotationSymmetry(PhysicalSymmetry):
    ...
    def generators(self) -> list[Any]:
        ...
        n = self._dimension
        return [
            AngularMomentumGenerator(i, j) for i in range(n) for j in range(i + 1, n)
        ]
...
class TimeTranslationSymmetry(PhysicalSymmetry):
    ...
    def create_operation(
        self, params: SymmetryParameters
...
class TimeReversalSymmetry(PhysicalSymmetry):
    ...
    def create_operation(
        self, params: SymmetryParameters = None
...
class CompositeSymmetry(PhysicalSymmetry):
    ...
    def create_operation(
        self, params: list[SymmetryParameters]
    ) -> "CompositeOperation":
        """创建组合操作"""
        operations = [s.create_operation(p) for s, p in zip(self._symmetries, params)]
