...
class ScalarField(Field):
    ...
    def gradient(self, position: np.ndarray) -> np.ndarray:
        ...
        for i in range(dim):
            ...
            pos_minus[i] -= epsilon
            grad[i] = (self.evaluate(pos_plus) - self.evaluate(pos_minus)) / (
                2 * epsilon
            )
        return grad
    ...
    def laplacian(self, position: np.ndarray) -> float:
        ...
        for i in range(len(position)):
            ...
            pos_minus[i] -= epsilon
            laplacian += (
                self.evaluate(pos_plus) - 2 * phi + self.evaluate(pos_minus)
            ) / (epsilon**2)
...
class VectorField(Field):
    ...
    def curl(self, position: np.ndarray) -> np.ndarray:
        ...
        for i in range(3):
            ...
            result[i] = (
                self.evaluate(pos_j_plus)[k]
                - self.evaluate(pos_j_minus)[k]
                - self.evaluate(pos_k_plus)[j]
                + self.evaluate(pos_k_minus)[j]
            ) / (2 * epsilon)
        return result
...
class ElectromagneticField(VectorField):
    ...
    def __init__(
        ...
        super().__init__(
            lambda x: np.concatenate([self._electric_field(x), self._magnetic_field(x)])
        )
    ...
    def tidal_force(self, position: np.ndarray) -> np.ndarray:
        ...
        for i in range(dim):
            for j in range(dim):
                ...
                pos_mm[j] -= epsilon
                tidal[i, j] = (
                    self.evaluate(pos_pp)
                    - self.evaluate(pos_pm)
                    - self.evaluate(pos_mp)
                    + self.evaluate(pos_mm)
                ) / (4 * epsilon**2)
        return tidal
    ...
    def field_strength(self, position: np.ndarray) -> np.ndarray:
        ...
        for mu in range(dim):
            for nu in range(mu + 1, dim):
                ...
                dA_nu_dmu = (
                    A_mu_plus[nu::dim][:gauge_dim]
                    if len(A_mu_plus) >= dim * gauge_dim
                    else A_mu_plus[nu]
                ) - (
                    A_nu_minus[nu::dim][:gauge_dim]
                    if len(A_nu_minus) >= dim * gauge_dim
                    else A_nu_minus[nu]
                )
                dA_mu_dnu = (
                    A_nu_plus[mu::dim][:gauge_dim]
                    if len(A_nu_plus) >= dim * gauge_dim
                    else A_nu_plus[mu]
                ) - (
                    A_mu_minus[mu::dim][:gauge_dim]
                    if len(A_mu_minus) >= dim * gauge_dim
                    else A_mu_minus[mu]
                )
                ...
                commutator = (
                    np.dot(A_mu_arr, A_nu_arr) - np.dot(A_nu_arr, A_mu_arr)
                    if hasattr(A_mu_arr, "__len__") and len(A_mu_arr) > 1
                    else 0
                )
...
class SpinorField(Field):
    ...
    def __init__(
        self, spinor_function: Callable[[np.ndarray], np.ndarray], mass: float = 0.0
    ):
    ...
    def dirac_equation_residual(
        self, position: np.ndarray, time_derivative: float = 0.0
    ) -> np.ndarray:
        """计算 Dirac 方程残差
        ...
        if len(position) == 4:
            dirac_operator = (
                1j * self._gamma_matrices[0] * time_derivative
                + 1j
                * sum(
                    self._gamma_matrices[i + 1] * spatial_gradient[i] for i in range(3)
                )
            ...
            dirac_operator = (
                1j
                * sum(
                    self._gamma_matrices[i] * spatial_gradient[i - 1]
