...
PhysicalQuantity = float | complex | np.ndarray
...
class PhysicalObject(ABC):
    ...
    def get_representation_matrix(self, rep_type: str = "vector") -> np.ndarray | None:
        """获取对象在某表示下的矩阵形式
...
class PhysicalSpace(ABC):
    ...
    _metric_tensor: np.ndarray | None = None
    _connection: np.ndarray | None = None
    ...
    @property
    def metric_tensor(self) -> np.ndarray | None:
        """获取度规张量
    ...
    def metric_tensor(self, g: np.ndarray) -> None:
        ...
        if g.shape[0] != self.dimension():
            raise ValueError(
                f"度规张量维度 {g.shape[0]} 与空间维度 {self.dimension()} 不匹配"
            )
    ...
    def project_onto(self, v: np.ndarray, subspace_bases: list) -> np.ndarray:
        ...
        G = np.array(
            [[self.inner_product_with_metric(bi, bj) for bj in bases] for bi in bases]
        )
