...
class ElementaryParticle(PhysicalObject):
    ...
    def __init__(
        ...
        self._spin = spin
        self._position = (
            np.zeros(3) if position is None else np.array(position, dtype=float)
        )
        self._velocity = (
            np.zeros(3) if velocity is None else np.array(velocity, dtype=float)
        )
        self._momentum = (
            np.zeros(3) if momentum is None else np.array(momentum, dtype=float)
        )
    ...
    def transform(
        self, symmetry_operation: "SymmetryOperation"
    ) -> "ElementaryParticle":
        """在对称操作下变换
