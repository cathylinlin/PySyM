...
class SphericalHarmonics:
    ...
    def compute(l: int, m: int, theta: float, phi: float) -> complex:
        ...
        Y = phase * norm * P_lm * np.exp(1j * m * phi)

        return Y

    @staticmethod
    def compute_array(
        l: int, m: int, theta_array: np.ndarray, phi_array: np.ndarray
    ) -> np.ndarray:
        """批量计算球谐函数"""
        # 网格化
    ...
    @staticmethod
    def orthonormality_integral(l1: int, m1: int, l2: int, m2: int) -> float:
        """
        正交归一性积分
...
class SphericalHarmonicsAddition:
    ...
    def addition_theorem(
        l: int, theta1: float, phi1: float, theta2: float, phi2: float
    ) -> float:
        """
        球谐函数加法定理
        ...
        """
        # 计算夹角 γ
        cos_gamma = cos(theta1) * cos(theta2) + sin(theta1) * sin(theta2) * cos(
            phi1 - phi2
        )
...
class GauntCoefficient:
    ...
    def compute_numerical(
        l1: int, m1: int, l2: int, m2: int, l3: int, m3: int, n_points: int = 100
    ) -> float:
        """
        数值积分计算Gaunt系数
        """
        ...
        result, _ = dblquad(
            integrand, 0, 2 * pi, lambda x: 0, lambda x: pi, epsabs=1e-10, epsrel=1e-10
        )
...
class SphericalHarmonicsIntegral:
    ...
    @staticmethod
    def integrate_product(Y1: Callable, Y2: Callable, n_points: int = 200) -> complex:
        """
        数值积分两个球谐函数的乘积
        """
        from scipy.integrate import dblquad
        ...
        result, _ = dblquad(
            lambda th, ph: integrand(th, ph), 0, 2 * pi, lambda x: 0, lambda x: pi
        )
