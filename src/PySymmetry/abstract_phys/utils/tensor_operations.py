...
class WignerEckartTheorem:
    ...
    @staticmethod
    def matrix_element(reduced_me: complex,
                      j: float, m: float,
                      k: int, q: int,
                      j_prime: float, m_prime: float) -> complex:
        """
    ...
    @staticmethod
    def matrix_element_3j(reduced_me: complex,
                        j: float, m: float,
                        k: int, q: int,
                        j_prime: float, m_prime: float) -> complex:
        """
    ...
    @staticmethod
    def reduced_matrix_element(full_me: complex,
                               j: float, m: float,
                               k: int, q: int,
                               j_prime: float, m_prime: float) -> complex:
        """
...
class VectorOperator(IrreducibleTensorOperator):
    ...
    @classmethod
    def from_cartesian(cls, Vx: np.ndarray, Vy: np.ndarray, Vz: np.ndarray) -> List['VectorOperator']:
        """从笛卡尔分量创建球分量"""
...
class AngularMomentumOperators:
    ...
    def J_x(j: float) -> np.ndarray:
        """J_x = (J⁺ + J⁻)/2"""
        return (AngularMomentumOperators.J_plus(j) + 
                AngularMomentumOperators.J_minus(j)) / 2
    
    @staticmethod
    def J_y(j: float) -> np.ndarray:
        """J_y = (J⁺ - J⁻)/(2i)"""
        return (AngularMomentumOperators.J_plus(j) - 
                AngularMomentumOperators.J_minus(j)) / (2 * 1j)
...
class TensorProduct:
    ...
    @staticmethod
    def product(T1: IrreducibleTensorOperator, 
                T2: IrreducibleTensorOperator,
                result_rank: int,
                result_component: int) -> IrreducibleTensorOperator:
        """
        计算张量积
                ...
                for q1 in range(-self._t1.rank, self._t1.rank + 1):
                    ...
                    if abs(q2) <= self._t2.rank:
                        cg = ClebschGordan.compute(
                            self._t1.rank, q1,
                            self._t2.rank, q2,
                            self.rank, self.component
                        )
                        # 需要中间态求和
                        # 简化版本
                        result += cg * self._t1.matrix_element(bra, ket) *                                   self._t2.matrix_element(bra, ket)
    ...
    @staticmethod
    def scalar_product(T1: IrreducibleTensorOperator,
                       T2: IrreducibleTensorOperator) -> 'ScalarOperator':
        """
        标量积
...
class SixJApplications:
    ...
    @staticmethod
    def recoupling_coefficient(j1: float, j2: float, j12: float,
                               j3: float, J: float, j23: float) -> float:
        """
        重耦合系数
        ...
        # 重耦合系数与6j符号的关系
        return (-1)**(j1 + j2 + j3 + J) * np.sqrt(
            (2*j12 + 1) * (2*j23 + 1)
        ) * Wigner6j.compute(j1, j2, j12, j3, J, j23)
    
    @staticmethod
    def reduced_matrix_element_product(T1_rank: int, T2_rank: int,
                                       j1: float, j: float, j2: float,
                                       reduced_me1: complex, reduced_me2: complex) -> complex:
        """
        计算张量积的约化矩阵元
        """
        from .clebsch_gordan import Wigner6j, Wigner3j
        
        result = 0.0
        for j_prime in np.arange(abs(j1 - T1_rank), j1 + T1_rank + 1, 1):
            six_j = Wigner6j.compute(T1_rank, j1, j_prime, 
                                    j2, j, T2_rank)
