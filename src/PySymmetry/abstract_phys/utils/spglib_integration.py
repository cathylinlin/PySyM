...
from ..symmetry_environments.discrete_symmetries.space_group import (
    SpaceGroup,
    SpaceGroupOperation,
)
...
class CrystalStructure:
    ...
    magmoms: np.ndarray | None = None  # 磁矩（可选）
    ...
    def from_xyz(
        ...
        try:
            ...
            numbers = np.array(
                [getattr(elements, sym).number for sym in atomic_symbols]
            )
        except ImportError:
            atomic_to_number = {
                "H": 1,
...
class SpglibAdapter:
    ...
    def get_symmetry_operations(
        self, structure: CrystalStructure, symprec: float = 1e-5
        ...
        for rotation, translation in zip(
            ...
            op = SpaceGroupOperation(rotation=rotation, translation=translation)
    ...
    def get_symmetry_from_database(self, hall_number: int) -> dict[str, Any]:
        ...
        return {
            ...
            "arithmetic_crystal_class_number": dataset[
                "arithmetic_crystal_class_number"
            ],
            "arithmetic_crystal_class_symbol": dataset[
    ...
    def get_site_symmetry(
        self, structure: CrystalStructure, atom_index: int, symprec: float = 1e-5
    ) -> list[SpaceGroupOperation]:
        """
        获取特定原子位置的点群对称性
...
def analyze_crystal(
    structure: CrystalStructure, symprec: float = 1e-5
) -> dict[str, Any]:
    """
    一键分析晶体对称性
...
def example_usage():
    ...
    # 示例1: 面心立方铝
    lattice_fcc = np.array([[0, 2.0, 2.0], [2.0, 0, 2.0], [2.0, 2.0, 0]])
