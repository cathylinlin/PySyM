...
class ClebschGordan:
    ...
    @staticmethod
    def compute(j1: float, m1: float, j2: float, m2: float, 
    ...
    @staticmethod
    def _racah_formula(j1: float, m1: float, j2: float, m2: float,
                       j: float, m: float) -> float:
        """Racah公式实现"""
    ...
    @staticmethod
    def inverse_clebsch_gordan(j: float, m: float,
                              j1: float, j2: float) -> Dict[Tuple[float, float], float]:
        """
...
class Wigner3j:
    ...
    @staticmethod
    def compute(j1: float, j2: float, j3: float,
    ...
    @staticmethod
    def compute_direct(j1: float, j2: float, j3: float,
                       m1: float, m2: float, m3: float) -> float:
        """
        直接计算3j符号（不通过CG系数）
        ...
        prefactor *= sqrt(
            cached_factorial(t1) * cached_factorial(t2) *
            cached_factorial(t3) * cached_factorial(t4) *
            cached_factorial(t5) * cached_factorial(t6)
        )
...
class Wigner6j:
    ...
    @staticmethod
    def _racah_formula(j1: float, j2: float, j3: float,
                       j4: float, j5: float, j6: float) -> float:
        """Racah公式实现"""
        
        # 计算三角形乘积
        def tri_prod(a, b, c):
            return cached_factorial(int(a + b - c)) *                    cached_factorial(int(a - b + c)) *                    cached_factorial(int(-a + b + c))
        
        # 前置因子
        prefactor = sqrt(
            tri_prod(j1, j2, j3) * tri_prod(j1, j5, j6) *
            tri_prod(j4, j2, j6) * tri_prod(j4, j5, j3) /
            cached_factorial(int(j1 + j2 + j3 + 1))
        )
...
class Wigner9j:
    ...
    @staticmethod
    def compute(j11: float, j12: float, j13: float,
