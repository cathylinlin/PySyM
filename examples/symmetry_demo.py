...
def demo_square_well_symmetry():
    ...
    scene = (
        SceneBuilder("Square Well Symmetry")
...
def demo_double_well_tunneling():
    ...
    scene = (
        SceneBuilder("Double Well Tunneling")
        .add_electron(position=[0])
...
def demo_selection_rules():
    ...
    scene = (
        SceneBuilder("Selection Rules Demo")
        .add_electron(position=[0])
        .add_harmonic_potential(center=[0], k=1.0)
        .set_spatial_range(-10, 10)
        .set_grid_points(300)
    ...
    for rule in allowed[:8]:
        print(
            f"  |{rule['initial']}> -> |{rule['final']}>: Delta E = {rule['energy_gap']:.4f}"
        )
    ...
    for rule in forbidden[:5]:
        print(
            f"  |{rule['initial']}> -> |{rule['final']}>: parity {rule['initial_parity']:+.0f} -> {rule['final_parity']:+.0f}"
        )
...
def demo_abstract_integration():
    ...
    scene = (
        SceneBuilder("Abstract Integration Demo")
        .add_electron(position=[2.5])
